...
# ---------------------------- App init --------------------------
app = FastAPI(title="Polyvore Smart Compatibility Backend", version="3.0.0-smart")
...
INCOMPATIBLE_PAIRS = {'ALL-BODY': {'OUTERWEAR'}, 'OUTERWEAR': {'ALL-BODY'}}
...
if os.path.exists(COMPAT_INDEX_PATH):
    try:
        ...
        logger.error("Failed to load compatibility resources: %s", e)
...
@app.get("/version")
def version(): return {"app_version": app.version, "faiss_version": faiss.__version__}

# === ENDPOINT MỚI VỚI LOGIC "SMART FILTERING" ===
...
def get_compatible_items_smart_filter(
    ...
    if query_sub_cat in SUMMER_BEACH_SUBCATS:
        category_whitelist = SUMMER_BEACH_WHITELIST
        logger.info(f"Item {item_id} context: SUMMER/BEACH")
    elif query_sub_cat in WINTER_COLD_SUBCATS:
        category_whitelist = WINTER_WHITELIST
        logger.info(f"Item {item_id} context: WINTER/COLD")
    elif query_sub_cat in FORMAL_OFFICE_SUBCATS:
        category_whitelist = FORMAL_OFFICE_WHITELIST
        logger.info(f"Item {item_id} context: FORMAL/OFFICE")
    ...
    for iid in I[0]:
        ...
        if not main_cat or main_cat not in category_whitelist or main_cat in selected_categories or main_cat in blacklisted_categories:
            continue
...
def _perform_similarity_search(query_vec: np.ndarray, topk: int, rerank: bool, candidates: Optional[int] = None) -> List[SearchHit]:
    k = max(1, topk)
    k_candidates = candidates or (k * 20 if rerank else k)
    ...
    if rerank and _vectors is not None:
        ...
        final_indices = result_indices[:k]
        final_distances = D[0][:k]
...
def similar(
    item_id: str, topk: int = Query(10, ge=1), rerank: bool = Query(True), candidates: Optional[int] = None
):
...
async def search_image(
    file: UploadFile = File(...), topk: int = Query(10, ge=1), rerank: bool = Query(True), candidates: Optional[int] = None
):
